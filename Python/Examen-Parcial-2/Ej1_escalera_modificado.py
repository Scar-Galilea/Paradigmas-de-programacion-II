...
Este programa dibuja una escalera, en donde el usuario ingresa el número de escalones + 1.
Si el número es positivo, la escalera será ascendente. Un ejemplo cuando se ingresa un valor de  4 + 1:        _
_
 |_
   |_
     |_
...
Si el número es negativo, la escalera será descendente. Un ejemplo cuando se ingresa un valor de -4 -1:
         _|
      _|
    _|
  _|
...
def Negativo(Numero_de_filas):
    Numero_de_filas = (Numero_de_filas * - 1 ) + 1
...
while Numero_de_filas != 0: #Ciclo principal que continúa mientras no se ingrese un 0.
    ...
    Numero_de_filas = int(input("Ingresa el número de escalones (positivo - descendente  y negativo - ascendente) o ingresa un cero para salir:  "))
